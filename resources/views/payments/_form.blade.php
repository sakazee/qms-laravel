<div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
    <div class="min-w-0">
        ...
        <select name="partner_id"
                class="input select2 js-partner-due-select @error('partner_id') border-rose-400 @enderror"
                data-due-map='{{ json_encode($partnerDueMap ?? []) }}' required>
            <option value="">-- {{ app()->getLocale() === 'bn' ? 'অংশীদার নির্বাচন করুন' : 'Select Partner' }}--
            </option>
    ...
    <div class="min-w-0">
        ...
        <div class="relative">
            ...
            <input type="number" name="amount" value="{{ old('amount', $payment->amount ?? '') }}"
                   class="input pl-8 @error('amount') border-rose-400 @enderror" min="0.01" step="0.01" required>
        </div>
    ...
    <div class="min-w-0">
        ...
        <input type="date" name="payment_date"
               value="{{ old('payment_date', isset($payment) ? $payment->payment_date->format('Y-m-d') : date('Y-m-d')) }}"
               class="input @error('payment_date') border-rose-400 @enderror" required>
...
@push('scripts')
    <script>
        window.qmsOnReady(function ($) {
            const $row = $('.js-partner-due-row');
            const $input = $('input[name="amount"]');
            if ($row.length === 0) return;

            const $select = $('.js-partner-due-select');
            let dueMap = {};
            try {
                dueMap = JSON.parse($select.attr('data-due-map') || '{}');
            } catch (e) {
                dueMap = {};
            }
