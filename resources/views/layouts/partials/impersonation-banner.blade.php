...
@endphp
@if($impersonatedUser)
<div class="flex flex-wrap items-center justify-center gap-2 border-b border-amber-200 bg-amber-50 px-4 py-2 text-[12.5px] font-semibold text-amber-900">
    ...
    <form action="{{ route('admin.impersonate.stop') }}" method="POST" class="m-0 inline">
        @csrf
        <button type="submit" class="rounded-full bg-amber-200 px-3 py-0.5 text-[12px] font-bold text-amber-900 transition-colors hover:bg-amber-300">
